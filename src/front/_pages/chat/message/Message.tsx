...
export const ChatMessage = (props: Props) => {
  ...
  return (
    <StyledMessage
      class={joinKnownClassNames([], 'chat-message')}
      sender-id={props.message.sentMemberId}
    >
      <div class={joinKnownClassNames(['ellipsis', 'full-width', 'margin-gap-b'], 'message-title')} />

      {props.message.replyMessageId != null && (
        <ChatReplyMessageHead
          message={props.message}
          replyMessageId={props.message.replyMessageId}
        />
      )}

      <div class={joinKnownClassNames([], 'chat-message-text')}>{props.message.text}</div>
